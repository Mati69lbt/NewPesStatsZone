...
import {
  buildAsistenciasGlobalRows,
  buildGoleadoresGlobalRows,
} from "../utils/goleadoresAsistenciasGlobalStats";
...
function GoleadoresAsistenciasGlobalPage() {
  ...
  const goleadoresRows = useMemo(
    () => buildGoleadoresGlobalRows(matches),
    [matches],
  );
  const asistenciasRows = useMemo(
    () => buildAsistenciasGlobalRows(matches),
    [matches],
  );
  const rows = isGoles ? goleadoresRows : asistenciasRows;

  const filteredRows = useMemo(
    () => rows.filter((row) => row.valor >= minimo),
    [rows, minimo],
  );
    ...
    <div className="flex min-h-dvh w-full max-w-full flex-col overflow-x-hidden bg-zinc-100 dark:bg-zinc-950">
      ...
      <main className="flex w-full max-w-full flex-1 flex-col items-center overflow-x-hidden px-2 py-10 sm:px-4">
        <div className="mx-auto flex w-full flex-col gap-6 md:max-w-2xl">
          <div className="text-center">
            <h1 className="flex items-center justify-center gap-2 text-2xl font-black uppercase tracking-wide text-zinc-900 dark:text-zinc-100 sm:text-3xl">
              <svg
                ...
                stroke="currentColor"
                strokeWidth="2"
                className="h-6 w-6 text-lime-500 sm:h-7 sm:w-7"
              >
                <path
                  strokeLinecap="round"
            ...
            <p className="text-sm font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
              {sortedRows.length}{" "}
              {sortedRows.length === 1 ? "jugador" : "jugadores"}
            </p>
          </div>
          ...
          <GoleadoresAsistenciasGlobalTable
            rows={sortedRows}
            modo={modo}
            sortKey={sortKey}
            sortDir={sortDir}
