        ...
        <table className="w-full border-collapse text-xs md:text-sm">
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {rows.map((row, index) => (
              ...
              >
                ...
                {isGoles ? (
                  <>
                    <td className={`${NUM_TD_CLASSES} font-bold text-lime-600 dark:text-lime-400`}>{row.goles}</td>
                    ...
                    <td className={NUM_TD_CLASSES}>
                      {row.x2 > 0 ? (
                        <span className="inline-flex items-center justify-center rounded-full bg-sky-500/15 px-1.5 py-0.5 text-[10px] font-bold text-sky-600 dark:text-sky-400 md:text-xs">
                          {row.x2}
                        </span>
                      ) : (
                    ...
                    <td className={NUM_TD_CLASSES}>
                      {row.x3 > 0 ? (
                        <span className="inline-flex items-center justify-center rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-bold text-amber-600 dark:text-amber-400 md:text-xs">
                          {row.x3}
                        </span>
                      ) : (
                        <span className="text-zinc-400 dark:text-zinc-600">-</span>
          ...
          <tfoot>
            <tr className="border-t-2 border-zinc-200 bg-gray-50 text-[10px] font-bold uppercase text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 md:text-xs">
              ...
              {isGoles ? (
                <>
                  <td className={`${NUM_TD_CLASSES} text-lime-600 dark:text-lime-400`}>{totals.goles}</td>
                  <td className={NUM_TD_CLASSES} />
                  <td className={`${NUM_TD_CLASSES} text-sky-600 dark:text-sky-400`}>{totals.x2}</td>
                  <td className={`${NUM_TD_CLASSES} text-amber-600 dark:text-amber-400`}>{totals.x3}</td>
                </>
              ) : (
                <>
