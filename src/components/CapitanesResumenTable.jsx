...
function CapitanCard({ row, index }) {
  return (
    <div className="w-full rounded-lg border border-zinc-700 bg-zinc-900 p-3">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-zinc-500">{row.club || 'Sin club'}</p>
          <p className="text-[10px] text-zinc-600">{row.years || '—'}</p>
        </div>
  ...
  return (
    <>
      <div className="flex w-full flex-col gap-3 sm:hidden">
        {rows.map((row, index) => (
          <CapitanCard key={row.key} row={row} index={index} />
        ))}
      </div>
          ...
          <table className="w-full min-w-[820px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-zinc-700 bg-zinc-800 text-left text-[11px] font-bold uppercase tracking-wide text-zinc-400">
                <th className="px-3 py-3">Capitán</th>
              ...
              {rows.map((row, index) => (
                ...
                >
                  <td className="px-3 py-3">
                    ...
                    <p className="text-xs text-zinc-500">{row.club || 'Sin club'}</p>
                    <p className="text-[10px] text-zinc-600">{row.years || '—'}</p>
                  </td>
